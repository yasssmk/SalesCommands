    ...
    def handle(self, *args, **options):
        ...
        try:
            with transaction.atomic():
                ...
                if create_admin:
                    if not admin_role:
                        raise CommandError('Cannot create admin user: Admin role not available')
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f'\n{"="*50}\n'
                        f'Tenant "{name}" successfully created!\n'
                        f'{"="*50}'
                    )
                )
                ...
                if not create_admin:
                    self.stdout.write(
                        self.style.WARNING(
                            '\nNote: No admin user created. You can create one later with:\n'
                            f'python manage.py create_tenant_admin --tenant-id {tenant.id}'
                        )
                    )
