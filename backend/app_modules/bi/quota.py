...
target_type mapping:
- closed_won   -> Sum(DecisionCycle.estimated_value) WON in the period (by outcome_date)  [flow]
- pipeline     -> Sum(DecisionCycle.estimated_value) OPEN right now (outcome NULL)          [STOCK — no period]
...
def compute_quota_current_value(quota) -> Optional[float]:
    ...
    if target_type == 'pipeline':
        ...
        agg = DecisionCycle.objects.filter(
    ...
    if target_type == 'closed_won':
        ...
        agg = qs.aggregate(total=Sum('estimated_value'))
        return float(agg['total'] or 0)
