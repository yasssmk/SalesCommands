...
class ActivityDecisionStepSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = DecisionStep
        # Step status is DERIVED on read (no stored column since migration 0019) —
        # never a field here, mirroring decision_cycles' StepMinimalSerializer.
        fields = ['id', 'name', 'stage', 'stage_display']
        read_only_fields = fields
...
class ActivityCreateSerializer(ClientScopeManager.SerializerMixin, serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if next_step_signal:
            ...
            # S13: carry the signal's suggested_objective onto the activity's
            # call_to_action (the unified "Activity Objective"). The objective
            # is generated by the next-steps extraction prompt. An explicit
            # call_to_action in the payload wins over the signal default.
            if not validated_data.get('call_to_action') and next_step_signal.suggested_objective:
                validated_data['call_to_action'] = next_step_signal.suggested_objective
...
class ActivityUpdateSerializer(ClientScopeManager.SerializerMixin, serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        try:
            ...
            # =================================================================
            # CAMPAIGN-LOCKED FIELDS (COND-1)
            # =================================================================
            # A campaign activity has date / contacts / owner / invited set by the
            # campaign; they must not be changed via a user PATCH. Title, type,
            # objective and description stay editable. Non-campaign activities are
            # untouched. The cascade writes on the model directly (activity.complete
            # / .save / .set) — never through this serializer — so it is unaffected.
            if instance.campaign_id:
                locked = {
                    'scheduled_date', 'scheduled_time', 'due_date',
                    'contact_ids', 'owner_id', 'invited_user_ids',
                }
                if locked & set(attrs.keys()):
                    raise StandardizedValidationError(
                        CoreErrorMessages.INVALID_DATA.format(
                            detail='This field is set by the campaign and cannot be edited'
                        )
                    )
