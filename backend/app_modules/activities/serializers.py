...
class ActivityCreateSerializer(ClientScopeManager.SerializerMixin, serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if next_step_signal:
            ...
            # call_to_action (the unified "Activity Objective"). An explicit
            # call_to_action in the payload wins over the signal default.
            #
            # DORMANT CAPABILITY — keep this branch. It is the intended
            # gateway that turns a next-step's proposed objective into the
            # materialised activity's call_to_action. It is a NO-OP today
            # because suggested_objective is never populated (the extraction
            # prompt does not emit it yet — TD, next-step objective
            # generation); it becomes live once the prompt fills the field.
            # Do NOT remove it (and if ever removed, remove the model field in
            # the same change to avoid referencing a dropped attribute).
