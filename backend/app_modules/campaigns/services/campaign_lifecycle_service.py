...
class CampaignLifecycleService:
    ...
    def _cancel_planned_activities(self, campaign):
        ...
        Activities are kept so the prospect account keeps a full trace of what
        was planned. Only PLANNED/ON_HOLD activities NOT linked to a decision
        cycle are marked CANCELLED; activities linked to a decision cycle are
        left untouched (locked rule: never touch a deal's activities), and
        already-terminal activities (COMPLETED/CANCELLED) are preserved as-is.
