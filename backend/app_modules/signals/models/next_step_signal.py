...
class NextStepSignal(BaseSignal):
    ...
    # DORMANT CAPABILITY — do NOT remove. A NextStepSignal is meant to
    # carry the OBJECTIVE of the activity it proposes (what it is for, the
    # stakes — prospect + commercial angle), which pre-fills
    ...
    # The field is wired for that mapping but NOT fed yet: the extraction
    # prompt does not emit it, so the mapping is currently a no-op. LLM
    # population is deferred — TD (next-step objective generation).
