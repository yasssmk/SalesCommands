...
### Sprint Bloc B / UX Activity ✅ (chantier UX Activity — EN COURS) — Section Header + Context + chantier Outcome + conditionnalité campagne (branche `feat/ux-activity-context`, `68b4c01c`→`f168b69a`)
⚠️ **Bloc B seulement — le chantier UX Activity N'EST PAS clos.** Header + Context + drawer edit + geste Outcome + verrou campagne sont livrés sur la branche ; les sections **Signals → Next step → Preparation/Source** restent à faire (Bloc C+).
- **Objectif** : livrer le **contenu Activity** des sections **Header** et **Context**, le **drawer d'édition** de l'activité, le **geste Outcome** (compléter une activité) et la **conditionnalité campagne** (verrou d'édition), sur les fondations UX Activity (thème aphoriQ + coque drawer unique).
- **Livré** (chaque sous-étape : reproduction ROUGE par le vrai chemin + sonde de non-vacuité par édition ciblée, STOP + validation PO) :
  - **Header** : `WorkspaceHeader` partagé (slots opaques + titre optionnel) ; chip statut `StatusPill` (contour / fond / texte en tokens) ; tuile icône type dimensionnée par `iconSizes` ; titre en lecture seule ; **menu ⋮ adaptatif** (Planned : Complete + Edit + Cancel + Delete ; Completed/Cancelled : Reopen + Edit + Delete). **Fix `Activity.is_overdue` = date EFFECTIVE** (scheduled OU due, COMPLETED/CANCELLED → non overdue ; `app_modules/activities/models.py:424-442`), aligné sur `StepStatusDerivationService`.
  - **Context** : affichage thémé aphoriQ (remplace l'ancien onglet, plus rendu) ; provenance intégrée ; call-to-action prioritaire ; People sans avatar + job title.
  - **Coque drawer** : arrondie détachée, fond anthracite, alignée en haut sur le header (pas de margin-top), titre optionnel porté par la coque (Option A) ; overlay avec focus-trap relâché pour les date pickers.
  - **Drawer edit activity** : primitives partagées `DrawerContentLayout` + `InlineEditableValue` (double-clic-pour-éditer, brouillon, Save/Cancel global) ; sections title / activity_type / date (toggle ✓✗ scheduled↔due, exclusif côté front) / objective / description / People (owner slot / invited / contacts, avec **déduplication**) ; câblé au ⋮ Edit.
  - **Outcome** : source unifiée **11 outcomes** + mapping `type → outcomes` (`frontend/src/utils/outcomes.js`) ; composants partagés `OutcomeSelector` (pills) + `OutcomeChip` ; **drawer outcome** (callback conditionnel campagne, date obligatoire si CALLBACK_REQUESTED) ; geste **Complete** E2E via `/complete/` (revalidation de la playlist si activité de campagne).
  - **Conditionnalité campagne** : verrou **front** (lecture seule) + garde **backend** — `scheduled_date` / `scheduled_time` / `due_date` / `contact_ids` / `owner_id` / `invited_user_ids` non éditables sur une activité de campagne (400 métier propre) ; `title` / `activity_type` / objective / `description` restent éditables. Prédicat = `campaign_id` (backend) / `Boolean(campaign_detail)` (front).
  - **Backend** : déduplication des ids (`invited_user_ids` / `contact_ids`) avant contrôle de cardinalité ; garde des champs campagne dans `ActivityUpdateSerializer.validate()` ; `is_overdue` effective.
- **Migrations** : **aucune** (front + serializer/property backend, aucun changement de schéma).
- **Décision produit ACTÉE** : `invited_user_ids` verrouillé sur une activité de campagne est **DÉFINITIF** (campagne = prospection solo) — ce n'est pas une dette.
- **Validation** : `pytest` `tests/activities/test_campaign_locked_fields.py`, `tests/activities/test_people_dedup_update.py`, `tests/notifications/test_emitter_activity_invitation.py` **verts (lancés par le PO)** ; `vitest` **1146 passed (161 fichiers)** ; **smoke PO front OK**.
- **Dette fermée** : **aucune** (le fix `is_overdue` effective n'avait pas de TD ouvert — livré directement).
- **Dette ajoutée** : **TD-222** (S2d drawer fiche contact), **TD-223** (unif playlist `CampaignOutcomeModal` → drawer outcome), **TD-224** (migration 3 chips outcome → `OutcomeChip`), **TD-225** (migration `ActivityCompleteModal` → drawer outcome), **TD-226** (code mort `ActivityOverviewTab` + `CollapsibleSection`), **TD-227** (token dark-surface aphoriQ), **TD-228** (provenance activité issue d'un DC / next-step non tracée), **TD-229** (post_save sur modèle `Activity` MORT), **TD-230** (`create_with_entities` ne valide pas l'exclusivité/au-moins-une des dates).
- **Prochain jalon** : sections dans l'ordre — **(1) Contact** (fiche contact, TD-222) → **(2) Signals** → **(3) Next step** (signal + création manuelle d'activité) → **(4) Transcript & notes** → **(5) Preparation / Email generation**, même discipline E2E. **UX Activity reste OUVERT.**

### Sprint Section Contact / UX Activity ✅ (chantier UX Activity — EN COURS) — Fiche contact (lecture) + fiche user + drawer edit contact + rôle DC éditable (branche `feat/ux-activity-contact`, `e8bfb41a`→`2fadd812`)
⚠️ **Section Contact seulement — le chantier UX Activity N'EST PAS clos.** La 1ʳᵉ section du contenu Activity (Contact) est livrée sur la branche ; les sections **Signals → Next step → Transcript/notes → Preparation** restent à faire.
- **Objectif** : livrer la **section Contact** au clic sur une personne de la section Context — fiche **contact** (lecture) et fiche **user** (membre interne, lecture), et le **drawer d'édition contact** (identité + coordonnées + rôle dans la décision quand on est dans un DC), sur les fondations UX Activity (coque drawer unique + `DrawerContentLayout` + `InlineEditableValue`).
- **Livré** (chaque sous-étape : reproduction ROUGE par le vrai chemin + sonde de non-vacuité par édition ciblée, STOP + validation PO) :
  - **Fiche contact (lecture)** : identité (avatar initiales + nom + « job · département ») ; coordonnées en **2 colonnes, tout affiché même vide** (placeholder « No email / No phone / No LinkedIn ») via `CoordinateRow` ; encart **« Involved in N activities in this deal »** dès qu'un DC existe (N lu de DC people, entrée `qualified` OU `unqualified`) ; **rôle dans la décision** lu de DC people (absent hors DC) ; icône **✎** (édition) tout à droite ; lien **« See signals »** inerte (futur). Ouverte au clic sur un contact externe du Context.
  - **Fiche user (lecture pure)** : nom réel + « role · team » + email (2 colonnes), **aucune action** (l'édition user est réservée aux admins ailleurs). Ouverte au clic sur un membre de l'internal team (owner / invited). Lit les deux formes du user (contexte auth `name`/`role` vs serializer API `full_name`/`role_name`/`team_name`).
  - **`CoordinateRow`** : primitive d'affichage 2 colonnes (icône + label / valeur ou placeholder), **partagée** par les fiches contact et user.
  - **Drawer edit contact** : sectionné comme le drawer edit activity — **Identité** (first/last* + job + **département en menu déroulant** `standard_department`), **Contact** (email/phone/linkedin inline, vides envoyés en **`""` (pas `null`)** — le write serializer accepte `blank` mais pas `null`), et **Role in the decision** (rôle + influence) **ÉDITABLE uniquement dans un DC**. Save → `updateContact` (**sans `account_id`**) ; le rôle → `createSignal("people", … MANUAL)` (flux de l'onglet DC People) + revalidation DC people, **uniquement si le rôle/influence a changé**. Ouvert par le ✎ de la fiche.
  - **`DrawerContentLayout`** : barre **Save/Cancel rendue optionnelle** (CT-1) — le layout sert désormais aussi les contenus **lecture** (fiches) sans barre.
  - **Backend** : `activity_count` exposé aussi sur les entrées **`qualified`** de DC people (CT-2a) — comptage DC-scopé partagé qualified/unqualified dans `PeopleConsolidationService`, **sans filtre de statut** (completed + futures + cancelled).
  - **Transverse** : fermeture du drawer **au changement de route** (`WorkspaceDrawerProvider` observe `usePathname`) ; **fix du flash « not found »** sur les pages compte/DC workspace (spinner tant que la donnée n'est pas résolue — la clé SWR nulle pendant l'hydratation `tenantId` n'est plus prise pour « introuvable »).
- **Migrations** : **aucune** (front + property/service backend `activity_count`, aucun changement de schéma).
- **Décision produit ACTÉE** : le **rôle DC est ÉDITABLE** depuis le drawer edit contact (via `PeopleSignal` MANUAL, flux DC People) — **INVERSE** la décision antérieure « lecture seule ». La décision antérieure (rôle en **LECTURE SEULE**, principe « ne pas mélanger contact et signal ») a été **RÉVISÉE en session** : le rôle est désormais **ÉDITABLE** via `PeopleSignal` MANUAL. **Revirement volontaire, pas un oubli.** L'influence éditée reste la propriété **per-DC** (`PeopleSignal.influence`), cohérent avec TD-213.
- **Validation** : `vitest` **1200 passed (170 fichiers)**, 0 erreur ; `pytest tests/decision_cycles/` **vert (lancé par le PO, 348 passed)** pour `activity_count` (CT-2a). **Smoke PO à faire.**
- **Dette fermée** : **TD-222** (drawer fiche contact — clic Context inerte) → **RESOLVED** (fiche contact + câblage du clic livrés).
- **Dette ajoutée** : **TD-231** (rôle DC empilé — pas d'endpoint update/replace), **TD-232** (lien « See signals » inerte), **TD-233** (fix racine `tenantKey` — clé-nulle transitoire vs légitime ; flash « not found » corrigé en LOCAL), **TD-234** (`useGetUser` — incohérence de forme auth vs serializer). **MAJ TD-213** (influence_level durable éditable = évolution, non traitée en V0).
- **Prochain jalon** : sections restantes dans l'ordre — **(2) Signals** → **(3) Next step** → **(4) Transcript & notes** → **(5) Preparation / Email generation**, même discipline E2E. **UX Activity reste OUVERT.**
...
- **Livré** (chaque sous-étape : premisses prouvées file:line, RED prouvé par le vrai chemin + non-vacuité par mutation ciblée, 0 hex/px en dur, commit fichier-par-fichier, STOP + validation PO) :
  ...
  - **S2 — Détail sur le châssis (Activity)** : `PainDetailView` (miroir d'`ObjectiveDetailView`) routé **Activity-only** (`signalType === "pain" && !leadingAction && !trailingAction` → le cluster DC reste flush) ; box récap extraite en `SignalSummaryBox` partagé ; en-tête possédé par la coque ; `related_techstack_mention` + scope + départements en lecture.
  - **S3 — Edit sur le châssis (Activity)** : `EditPainContent` (miroir d'`EditObjectiveContent`) ; **scope EXCLUSIF** Company/Department en vraies pills (`ObjectiveScopePill`/`objectiveScope`, non modifié) ; **multi-select département M2M** (pills avec ×, « + add department », menu excluant les déjà-choisis) ; bascule Company **masque sans effacer** ; payload dérive `target_departments` du scope (Company → `[]`), **aucune FK**.
  - **S4 — Recâblage dette M2M front** : `SignalLine.getScopeLabel` + `utils/groupedSignalFilter.js` lisent la **LISTE** `target_departments` (Pain/Impact/Constraint) tout en gardant la FK `target_department` (Objective/People) ; tests migrés FK→M2M (ids int **et** string normalisés).
- **Bug de fond corrigé (multi-select département)** : mismatch de type **int (endpoint choices) vs string (payload)** → `indexOf`/`===` strict de MUI Select échoue à toggler. Fix = stringifier les valeurs d'option / stocker `{value,label}` + extraire les ids au payload. **⚠️ Le MÊME défaut est LATENT sur TechStack `usage_departments`** → **TD-241** (nouveau).
- **Migrations** : **aucune** (front + serializer Pain writable, aucun changement de schéma).
- **Validation** : `vitest` **1385 passed (183 fichiers)**, 0 erreur, lint propre ; backend : `test_pain_impact_departments_m2m.py` (write M2M création/PATCH/PATCH `[]`) **VERT**. **Smoke PO OK (détail + edit Pain, surface Activity) ; reste le filtre département à smoker sur DC/Account → TD-243.**
