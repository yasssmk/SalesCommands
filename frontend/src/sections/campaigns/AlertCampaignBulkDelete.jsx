// frontend/src/sections/campaigns/AlertCampaignBulkDelete.jsx
...
import { bulkDeleteCampaigns } from 'api/campaigns/campaigns';
 ...
 *
 * Confirmation dialog for bulk campaign deletion.
 * Mirrors AlertTerritoryBulkDelete for consistency.
 *
 * @param {Array} selectedIds - Array of campaign UUIDs to delete
 ...
 */
export default function AlertCampaignBulkDelete({ selectedIds, open, handleClose, onDeleteComplete }) {
  const [deleting, setDeleting] = useState(false);
  const [processing, setProcessing] = useState(false);
  const [hadTimeout, setHadTimeout] = useState(false);

  const campaignCount = selectedIds?.length || 0;

  const handleSyncComplete = useCallback(() => {
    if (hadTimeout) {
      displaySuccessSnackbar(
        `${campaignCount} campaign${campaignCount > 1 ? 's' : ''} deleted successfully`
      );
    }
    ...
    onDeleteComplete?.();
    setHadTimeout(false);
  }, [hadTimeout, campaignCount, handleClose, onDeleteComplete]);
  ...
  const deleteHandler = async () => {
    ...
    try {
      ...
      const res = await bulkDeleteCampaigns(
        selectedIds,
        'partial',
        onSyncProgress,
        onSyncComplete
      ...
      if (res?.data?.__is202) {
        console.log('[AlertCampaignBulkDelete] 202 Accepted → entering processing state');
        setProcessing(true);
        return;
      }
      ...
      if (res?.success === true) {
        ...
        console.log('[AlertCampaignBulkDelete] Timeout detected, sync will start');
        setHadTimeout(true);
        setProcessing(true);
      } else {
      ...
      console.error('[AlertCampaignBulkDelete] Exception:', err);
...
AlertCampaignBulkDelete.propTypes = {
