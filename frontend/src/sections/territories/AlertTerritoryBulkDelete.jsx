// frontend/src/sections/territories/AlertTerritoryBulkDelete.jsx
...
import { bulkDeleteTerritories } from 'api/territories/territories';
 ...
 * 
 * Confirmation dialog for bulk territory deletion.
 * Follows AlertAccountBulkDelete pattern for consistency.
 * 
 * @param {Array} selectedIds - Array of territory UUIDs to delete
 ...
 */
export default function AlertTerritoryBulkDelete({ selectedIds, open, handleClose, onDeleteComplete }) {
  const [deleting, setDeleting] = useState(false);
  const [processing, setProcessing] = useState(false);
  const [hadTimeout, setHadTimeout] = useState(false);
  
  const territoryCount = selectedIds?.length || 0;

  const handleSyncComplete = useCallback(() => {
    if (hadTimeout) {
      displaySuccessSnackbar(
        `${territoryCount} territor${territoryCount > 1 ? 'ies' : 'y'} deleted successfully`
      );
    }
    ...
    onDeleteComplete?.();
    setHadTimeout(false);
  }, [hadTimeout, territoryCount, handleClose, onDeleteComplete]);
  ...
  const deleteHandler = async () => {
    ...
    try {
      ...
      const res = await bulkDeleteTerritories(
        selectedIds,
        'partial',
        onSyncProgress,
        onSyncComplete
      ...
      if (res?.data?.__is202) {
        console.log('[AlertTerritoryBulkDelete] 202 Accepted → entering processing state');
        setProcessing(true);
        return;
      }
      ...
      if (res?.success === true) {
        ...
        console.log('[AlertTerritoryBulkDelete] Timeout detected, sync will start');
        setHadTimeout(true);
        setProcessing(true);
      } else {
      ...
      console.error('[AlertTerritoryBulkDelete] Exception:', err);
...
AlertTerritoryBulkDelete.propTypes = {
