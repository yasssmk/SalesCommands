// frontend/src/sections/activities/signals/wizard/sections/PainSection.jsx
/**
 ...
 *
 * Source line provenance (post-standardisation refactor)
 * ------------------------------------------------------
 * The form no longer captures `source_contact`. We derive a
 * compact "Contact · Date" line from the linked activity:
 *   - contact label : single contact name when activity.contacts.length === 1,
 *                     "<N> contacts" when > 1, omitted otherwise
 *   - date          : activity.scheduled_date or activity.completed_at
 *
 * The activity object is the full payload captured by AsyncActivitySelect,
 * with `contacts` already nested (backend ActivityCompactSerializer).
 */
function StagedPainCard({ signal, choices, onToggleStatus, onEdit, onRemove }) {
  ...
  const sourceLine = useMemo(() => {
    ...
    if (contacts.length === 1) {
      const c = contacts[0] ?? {};
      contactLabel = `${c.first_name || ""} ${c.last_name || ""}`.trim();
    ...
    const parts = [];
    if (contactLabel) parts.push(contactLabel);
    ...
    >
      <Stack direction="row" spacing={1.5} alignItems="flex-start">
        ...
        <Stack spacing={0.75} flex={1} minWidth={0}>
          ...
          {/* Source line — "Nicky Larson · 2026-04-15" derived from activity */}
          {sourceLine && (
