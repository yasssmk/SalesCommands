...
import {
  WORKSPACE_TABS,
  DEFAULT_TAB,
} from "sections/accounts/workspace/AccountTabs";
import {
  useGetAccountWorkspace,
...
export default function AccountWorkspacePage() {
  ...
  const { account, stats, workspaceLoading, workspaceError, mutateWorkspace } =
    useGetAccountWorkspace(accountId);

  const { industries, choicesLoading } = useGetAccountChoices();
  ...
  const handleTabChange = (newTab) => {
    ...
    router.push(`/accounts/${accountId}?${params.toString()}`, {
      scroll: false,
    });
  };
  ...
  const handleCycleChange = useCallback(
    (cycleId) => {
      ...
      router.replace(`/accounts/${accountId}?${params.toString()}`, {
        scroll: false,
      });
    },
    [router, accountId],
        ...
        <MainCard>
          <Stack spacing={2} alignItems="center" py={4}>
            ...
            <Typography color="text.secondary">
              The account you are looking for does not exist or you don&apos;t
              have access.
            </Typography>
