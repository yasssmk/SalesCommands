...
const endpoints = {
  ...
  accountActiveEngagements: (id) =>
    `/company-accounts/${id}/active-engagements/`,
...
const buildUrlWithParams = (baseUrl, params = {}) => {
  ...
  if (
    filters.has_buying_decision !== undefined &&
    filters.has_buying_decision !== null
  ) {
  ...
  if (
    filters.has_qualification !== undefined &&
    filters.has_qualification !== null
  ) {
...
export function useGetAccounts(options = {}) {
  ...
  const urlWithParams = useMemo(() => {
    return buildUrlWithParams(endpoints.accounts, {
      page,
      pageSize,
      search,
      ordering,
...
/**
 * GET ACTIVE ENGAGEMENTS — active campaigns + open decision cycles for an account.
 * Used to detect concurrent engagements and surface alerts.
 *
 * @param {string} accountId - UUID of the account
 * @returns {Object} {activeCampaigns, activeDecisionCycles, summary, engagementsLoading, engagementsError}
 */
export function useGetActiveEngagements(accountId) {
  const { tenantId } = useAuth();

  const swrKey = useMemo(() => {
    if (!accountId || !isValidUUID(accountId)) return null;
    return tenantKey(endpoints.accountActiveEngagements(accountId), tenantId);
  }, [accountId, tenantId]);
  ...
  return useMemo(
    () => ({
      activeCampaigns: data?.data?.active_campaigns || [],
      activeDecisionCycles: data?.data?.active_decision_cycles || [],
...
export const bulkDeleteAccounts = async (
  ...
  try {
    console.log("[bulkDeleteAccounts] start", {
    ...
    if (invalidIds.length > 0) {
      console.error("[bulkDeleteAccounts] invalid UUIDs", {
    ...
    if (result?.data?.__is202) {
      ...
      if (finalResult?.data) {
        ...
        if (finalResult.data?.status === "pending") {
          console.log(
            "[bulkDeleteAccounts] ⏳ Polling reached max attempts, marking operation as pending",
          );
          return {
            ...finalResult.data,
            success: false,
    ...
    if (result.isTimeout) {
      console.log(
        "[bulkDeleteAccounts] ⏱️ Timeout detected in result, starting polling...",
      );
    ...
    if (result.data && typeof result.data === "object") {
      if (result.data.results || result.data.summary) {
        console.log(
          "[bulkDeleteAccounts] returning structured error from backend",
        );
        return {
          ...result.data,
          success: false,
    ...
    if (isTimeout) {
      console.log(
        "[bulkDeleteAccounts] ⏱️ Timeout detected, starting polling...",
      );
    ...
    return {
      ...
      summary: {
        ...
        deleted: 0,
        failed: accountIds?.length ?? 0,
      },
      results: {
        success: [],
        failed: (accountIds || []).map((id) => ({
          id,
...
export const bulkUpdateAccounts = async (
  ...
  try {
    ...
    const invalidIds = accountIds.filter((id) => !isValidUUID(id));
    if (invalidIds.length > 0) {
      console.error("[bulkUpdateAccounts] invalid UUIDs", {
        count: invalidIds.length,
      });
      return {
        success: false,
    ...
    if (result?.data?.__is202) {
      ...
      if (finalResult?.data) {
        ...
        if (finalResult.data?.status === "pending") {
          console.log(
            "[bulkUpdateAccounts] ⏳ Polling reached max attempts, marking operation as pending",
          );
          return {
            ...finalResult.data,
            success: false,
    ...
    if (result.isTimeout) {
      console.log(
        "[bulkUpdateAccounts] ⏱️ Timeout detected in result, starting polling...",
      );
    ...
    if (result.data && typeof result.data === "object") {
      if (result.data.results || result.data.summary) {
        console.log(
          "[bulkUpdateAccounts] returning structured error from backend",
        );
        return {
          ...result.data,
          success: false,
    ...
    if (isTimeout) {
      console.log(
        "[bulkUpdateAccounts] ⏱️ Timeout detected, starting polling...",
      );
...
export const bulkCreateAccounts = async (
  ...
  try {
    console.log("[bulkCreateAccounts] start", {
    ...
    if (result?.data?.__is202) {
      ...
      if (finalResult?.data) {
        ...
        if (finalResult.data?.status === "pending") {
          console.log(
            "[bulkCreateAccounts] ⏳ Polling reached max attempts, marking operation as pending",
          );
          return {
            ...finalResult.data,
            success: false,
      ...
      if (finalResult && (finalResult.summary || finalResult.results)) {
        console.log(
          "[bulkCreateAccounts] ✅ Returning finalResult directly (no nesting)",
        );
        return finalResult;
      }

      console.error(
        "[bulkCreateAccounts] ❌ finalResult missing data structure",
      );
      return {
        success: false,
        message: "Polling completed but no result data",
    ...
    if (result.success) {
      console.log("[bulkCreateAccounts] ✅ Sync success", {
        status: result.status ?? 200,
        created:
          result.data?.summary?.success ?? result.data?.summary?.created ?? 0,
      });
    ...
    if (result.isTimeout) {
      console.log(
        "[bulkCreateAccounts] ⏱️ Timeout detected in result, starting polling...",
      );
    ...
    if (result.data && typeof result.data === "object") {
      if (result.data.results || result.data.summary) {
        console.log(
          "[bulkCreateAccounts] returning structured error from backend",
        );
        return {
          ...result.data,
          success: false,
    ...
    if (isTimeout) {
      console.log(
        "[bulkCreateAccounts] ⏱️ Timeout detected, starting polling...",
      );
